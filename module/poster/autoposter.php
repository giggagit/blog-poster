...
if ((isset($_REQUEST['wp'])) OR (isset($_REQUEST['bg']))) {
    ...
    if ($resultItem->num_rows > '0') {
        ...
        while ($itemRow = $resultItem->fetch_array(MYSQLI_ASSOC)) {
            ...
            if (isset($_REQUEST['wp'])) {
                ...
                if (is_numeric($wpPoster)) {
                    ...
                    $queryTask = "UPDATE scheduler SET last_fired = '" . $now . "', fire_time = '" . ($now + $cronjob + $randCron) . "' WHERE scriptpath like '%wpID=" . $wpID . "'";
                    $resultTask = $mysqli->query($queryTask) OR trigger_error($mysqli->error . "[$queryTask]");
                    echo 'Posted: ' . $item_productId;
                } else {
                    echo $wpPoster;
                }
            }
        ...
        if (isset($_REQUEST['wp'])) {
            $queryNoitem = "INSERT INTO error_log (ID, blogId, type, blog_url, reason, datetime) VALUES (NULL, '" . $wpID . "', 'wordpress', '" . $wp_blogurl . "', 'No products', '" . $now . "')";
            $resultNoitem = $mysqli->query($queryNoitem) OR trigger_error($mysqli->error . "[$queryNoitem]");
            echo 'No products';
        }
        
        if (isset($_REQUEST['bg'])) {
            $queryNoitem = "INSERT INTO error_log (ID, blogId, type, blog_url, reason, datetime) VALUES (NULL, '" . $bgID . "', 'blogger', '" . $bg_blogurl . "', 'No products', '" . $now . "')";
            $resultNoitem = $mysqli->query($queryNoitem) OR trigger_error($mysqli->error . "[$queryNoitem]");
                echo 'No products';
        }
    }
