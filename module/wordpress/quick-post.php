...
if (isset($_POST['action'])) {
    ...
    $getWP = slashes($_POST);
    $wp_blogurl = $getWP['blogurl'];
    $wp_username = $getWP['username'];
    $wp_password = $getWP['password'];
    ...
    foreach ($getWP['keywords'] as $keyGroups) {
        ...
        if (isset($connectProsperent['error'])) {
            ...
            $wpMsg = '<p>Post to wordpress: <strong>' . $getWP['blogurl'] . '</strong></p>';
        }
    }
    ...
    foreach($dataProsperent as $getData) {
        ...
        $wpPoster = wpPostXMLRPC($title, $contents, $wp_blogurl, $wp_username, $wp_password, $getItem['wp_category'], '', tag($item_keyword), $now, 'quick-post');
        $alertMsg = 'alert-success';
        if (is_numeric($wpPoster)) {
            ...
            $getProducts[] = '<p><span class="label label-success">Posted</span> ' . $item_keyword . ' <a href="' . $wp_blogurl . '/?p=' . $wpPoster .'" target="_blank">Views</a><p>';
        } else {
            $getProducts[] = '<p>' . $wpPoster . '</p>';
        }
    }
}
